    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-12 text-center">
          ...
          <span>{{ lisOfCategories.length + "-" + "Categories" }}</span>
        </div>
      </div>
      <div class="row">
        ...
        <div class="col-lg-12 text-center">
          <div class="btn btn-del" @click="deleteAllCateg()">Delete All</div>
        </div>
      </div>

      <div class="text-danger" v-if="successmessage">
        {{ successmessage }}
      </div>
    </div>
  </div>
...
import { deleteAllCategories } from "../Modules/Menue/deleteAllCategories";
export default {
  setup() {
    const addCateg = addCategory();
    const delAll = deleteAllCategories();
    const MyMenue = menu();
    return { ...addCateg, ...MyMenue, ...delAll };
  },
  name: "deleteAlCategoriesComp",
  components: {
    Header,
  },
  beforeMount() {
    this.moveById = this.$route.params.CategoryMenu;
    this.deleteAllInfo();
    this.canUserAccessThisLocation(this.userId, this.moveById, "home");
    this.getCategId();
    this.getAllrealtedItems();
    this.randomImages();
  },
};
</script>
